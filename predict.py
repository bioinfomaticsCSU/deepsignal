...
def predict(argv):
    ...
    with tf.Session() as sess:
        ...
        saver.restore(sess, LOG_DIR + 'final.ckpt')
        accuracy_list = []
        ...
        try:
            while True:
                ...
                feed_dict = {model.base_int: features['base'],
                             ...
                             model.keep_prob: 1.0}
                activation_logits, prediction, accuracy = sess.run(
                    [model.activation_logits, model.prediction, model.accuracy], feed_dict=feed_dict)
